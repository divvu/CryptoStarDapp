...
it('can add the star name and star symbol properly', async() => {
    ...
    let starLookUp = await instance.lookUptokenIdToStarInfo(tokenId);

    assert.equal(name, 'Star Token');
    assert.equal(symbol, 'STR');
    assert.equal(starLookUp, 'Test Star');
});
...
it('lets a user transfer a star', async() => {
    ...
    // 1. create a Star with different tokenId
    let instance = await StarNotary.deployed();
    await instance.createStar('Transfer a Star', tokenId, {from: user1});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    // 1. create a Star with different tokenId
    let instance = await StarNotary.deployed();
    await instance.createStar('Lookup a Star', tokenId, {from: user1});

    // 2. Call your method lookUptokenIdToStarInfo
    let starName = await instance.lookUptokenIdToStarInfo(tokenId, {from: user1});

    // 3. Verify if you Star name is the same
    assert.equal(starName, 'Lookup a Star');
});